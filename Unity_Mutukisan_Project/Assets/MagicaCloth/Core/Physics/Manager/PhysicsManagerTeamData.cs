            ...
            public void Execute(int teamId)
            {
                ...
                else
                {
                    ...
                    if (wdata.resetTeleport == 1)
                    {
                        ...
                        var angDiff = math.degrees(MathUtility.Angle(wdata.oldRotation, brot));
                        if (moveLen >= wdata.teleportDistance * tdata.scaleRatio || angDiff >= wdata.teleportRotation)
                        {
                            // テレポートモードにより処理を分岐
                    ...
                    else if (moveLen > 1e-06f && dtime > 1e-06f)
                    {
                        ...
                        //const float maxspeed = 10.0f; // 最大速度テスト m/s
