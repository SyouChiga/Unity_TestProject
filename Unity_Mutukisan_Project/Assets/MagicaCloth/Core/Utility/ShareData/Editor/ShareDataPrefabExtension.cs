...
namespace MagicaCloth
{
    ...
    internal class ShareDataPrefabExtension
    {
        ...
        static List<CoreComponent> processedList = new List<CoreComponent>();
        ...
        {
            //Debug.Log($"OnPrefabInstanceUpdate()->{instance.name} id:{instance.GetInstanceID()}");
        ...
        static void DelaySavePrefabAndConnect()
        {
            ...
            processedList.Clear();
            var prefabList = new List<GameObject>(prefabInstanceList);
            prefabInstanceList.Clear();

            prefabList.Reverse(); // 子から親の順に処理する
            foreach (var instance in prefabList)
            {
                if (instance)
                {
                    ...
                    //prefab = PrefabUtility.GetCorrespondingObjectFromOriginalSource(instance) as GameObject;
                    prefabPath = AssetDatabase.GetAssetPath(prefab);

                    if (prefab == null)
                    {
                        // プレハブ化前かつプレハブステージならばアクティブプレハブから取得する
        ...
        static void PrefabUpdate(GameObject prefab, GameObject target, string prefabPath)
        {
            ...
            //Debug.Log($"prefab name:{prefab.name} id:{prefab.GetInstanceID()}");
            //Debug.Log($"target name:{target.name} id:{target.GetInstanceID()}");
            ...
            // データコンポーネント収集
            var coreList = target.GetComponentsInChildren<CoreComponent>(true);
            if (coreList != null)
            {
                for (int i = 0; i < coreList.Length; i++)
                {
                    var core = coreList[i];
                    var nearRoot = PrefabUtility.GetNearestPrefabInstanceRoot(core);
                    string nearPrefabPath = string.Empty;
                    if (nearRoot)
                    {
                        //nearPrefabPath = AssetDatabase.GetAssetPath(nearRoot);

                        // 該当コンポーネントを内包するプレハブ
                        nearPrefabPath = PrefabUtility.GetPrefabAssetPathOfNearestInstanceRoot(nearRoot);
                    }
                    //Debug.Log($"core[{core.name}] nearPrefab:{nearRoot} prefabPath:{nearPrefabPath}");

                    // すでに処理済みならスルー
                    if (processedList.Contains(core))
                        continue;
                    processedList.Add(core);
                            ...
                            if (shareDatas != null)
                            {
                                foreach (var sdata in shareDatas)
                                {
                                    if (sdata)
                                    {
                                        ...
                                        //Debug.Log($"[{sdata.name}] prefabRoot:{PrefabUtility.GetOutermostPrefabInstanceRoot(sdata)}");
                                        ...
                                        else if (nearPrefabPath != AssetDatabase.GetAssetPath(sdata))
                                        {
                                            // 保存先プレハブが変更されている
