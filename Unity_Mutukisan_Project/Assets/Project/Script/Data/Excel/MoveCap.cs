...
public class MoveCap : MonoBehaviour
{
    ...
    float MoveCollisionCalc(Vector3 moveDir, float moveDistance, out Vector3 nextMoveDir)
    {
        ...
        var col = _collider;
        var size = _collider.transform.TransformVector(col.radius, col.height, col.radius);
        var radius = 0.5f + 0.1f;
        var height = size.y;
        var center = _collider.transform.TransformPoint(col.center);
        ...
        //指定した半径の球に当たるコライダー
        Collider[] colliders = Physics.OverlapCapsule(top, bottom, radius, CollisionLayerMask);
